...
| B — Hồ Văn Tâm | `app/logging_config.py` (đăng ký `scrub_event` trước JSONRenderer), `app/pii.py` (thêm `passport`, `address_vn`), `app/main.py` (log enrichment `user_id_hash`/`session_id`/`feature`/`model`/`env`) | `7433995`, `76de046`, `d3ce6a5`, `fc34782` | học được là phải che PII cho các trường thông tin nhạy cảm, phải định nghĩa rõ lỗi api khi gọi nếu không sẽ nhận lỗi chung chung, khó debug, biết các lỗi hay gặp và đọc log trường nào để tìm ra lỗi |
...
| D — Nguyễn Hoàng Bảo Minh | `config/slo.yaml` (4 SLI kèm giải thích), `config/alert_rules.yaml` (3 alert symptom-based), `docs/alerts.md` (runbook); **bonus:** cost guard `MAX_OUTPUT_TOKENS`, audit log riêng (`app/audit.py` → `data/audit.jsonl`), anomaly detector (`scripts/detect_anomalies.py`) | `29b0244`, `f5cf5c5`, `9e9ffb3` | Hiểu cách chuyển SLI thành SLO, alert và runbook; biết dùng cost guard để giảm tác động của cost spike, audit log để truy vết thay đổi vận hành và automation để phát hiện anomaly từ structured logs. |
